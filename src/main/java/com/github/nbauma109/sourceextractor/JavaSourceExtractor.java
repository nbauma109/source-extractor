...
public class JavaSourceExtractor {
    ...
    private static void extractAndSaveTypes(Path filePath) throws IOException {
        ...
        for (TypeDeclaration<?> type : compilationUnit.getTypes()) {
            ...
            if (type.isPublic() || fileWithoutExtension.equals(typeName)) {
                continue;
            }
            Optional<Range> typeRange = type.getRange();
            if (typeRange.isPresent()) {
                Range range = typeRange.get();
                List<String> typeLines = lines.subList(range.begin.line - 1, range.end.line);
                String originalText = String.join("\n", typeLines);

                int lineStart = type.getBegin().map(pos -> pos.line).orElse(1);
                String padding = "\n".repeat(lineStart - 1);
                String extractedContent = padding + originalText;
